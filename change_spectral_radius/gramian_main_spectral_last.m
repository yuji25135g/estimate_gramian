%function [train_X, train_Y, test_X, test_Y] = gramian_main_spectral_last(rho)
rho = 0.9;
%{
入力する変数
time：1データ当たりの時間発展
N_x：リザバーのノード数

スペクトル半径を引数にもつ
各データ最後の10点のみを用いて学習
timeは20で固定→いつか可変にしたい
%}


%データの読み込み
load('dataset2_from_data')

%{
%データのスケーリング
data_scale = 10^-3;
data = data_scale * sunspotData;
%}

%訓練・検証データ長
trainU_size = size(train_U);
trainD_size = size(train_D);
testU_size = size(test_U);
T_train = trainU_size(1,2);
T_test = testU_size(1,2);
time = 20;%1データあたりの時間発展


%各層のノード数
N_u = trainU_size(1,1);
N_x = 8000;
N_y = trainD_size(1,1);

%Winの設定
inputScale = 1;
Win = 2*inputScale*rand(N_x, N_u) - inputScale;


%Wの設定
density = 0.1;
W = zeros(N_x); %wの初期化　任意に設定する場合はここに代入
W = gen_randomW(N_x, density, W);
eigv_list = eig(W); %結合重み行列wの固有値
sp_radius = max(abs(eigv_list)); %現在のスペクトル半径（固有値の絶対値の最大値）
W = W * rho / sp_radius;  %任意のスペクトル半径rhoに合うようにwをスケーリング

%Woutの設定
ave = 0; %平均
sd = 1; %標準偏差
Wout = sd*randn(N_y, N_x) + ave;

%学習RC実行-------------------------------------------------------------------------------------------------------------------------------
trainData_num = 1;
disp("study")
study_X = [];
study_Y = [];
for i = 1: trainData_num
    u = train_U(:,:,i);
    [y_all, x_all] = RC(Win, W, Wout, u, N_u, N_x, N_y, T_train, zeros(N_x,1));
    %データの保存
    study_X(:,:,i) = x_all;
    study_Y(:,:,i) = y_all;
end

%線形回帰-----------------------------------------------------------------------------------------------------------------------------
%線形回帰を行うためのデータを形成
last10_study_X = [];
last10_train_D = [];
for i= 1: T_train/time
    last10_study_X = [last10_study_X, study_X(:,20*i-9:20*i)];
    last10_train_D = [last10_train_D, train_D(:,20*i-9:20*i)] ;
end
disp("linear")
b = 0.001; %正則化パラメータ
Wupd = Linear(last10_study_X, last10_train_D, b, N_x);


%訓練RC実行（訓練誤差を求めるため）---------------------------------------------------------------------------------------------------------
disp("training")
train_X = [];
train_Y = [];
for i = 1: trainData_num
    u = train_U(:,:,i);
    [y_all, x_all] = RC(Win, W, Wupd, u, N_u, N_x, N_y, T_train, zeros(N_x,1));
    %データの保存
    train_X(:,:,i) = x_all;
    train_Y(:,:,i) = y_all;
end

%ラベル出力----------------------------------------------------------------------------------------------------------------------
%モデル出力に対して
pre_train = zeros(N_y, T_train/time);
for i = 1: T_train/time
    tmp = train_Y(:,20*i-9:20*i); %1データに対する出力
    [~, max_index] = max(tmp); %最大出力を与えるノード番号の配列
    pre_train(mode(max_index), i) = 1; %各データに対するone-hotを格納した配列
end
%目標出力に対して
train_label = [];
for i = 1: T_train/time
    train_label = [train_label, train_D(:,i*time)];
end

%検証RC実行----------------------------------------------------------------------------------------------------------------------------------
testData_num = 1;
disp("test")
test_X = [];
test_Y = [];
for i = 1: testData_num
    u = test_U(:,:,i);
    [y_all, x_all] = RC(Win, W, Wupd, u, N_u, N_x, N_y, T_test, train_X(:,end,1));
    %データの保存
    test_X(:,:,i) = x_all;
    test_Y(:,:,i) = y_all;
end

%ラベル出力--------------------------------------------------------------------------------------------------------------------------
%モデル出力に対して
pre_test = zeros(N_y, T_test/time);
for i = 1: T_test/time
    tmp = test_Y(:,20*i-9:20*i); %1データに対する出力
    [~, max_index] = max(tmp); %最大出力を与えるノード番号の配列
    pre_test(mode(max_index), i) = 1; %各データに対するone-hotを格納した配列
end
%目標出力に対して
test_label = [];
for i = 1: T_test/time
    test_label = [test_label, test_D(:,i*time)];
end

%混同行列--------------------------------------------------------------------------------------------------------------------------
 plotconfusion(train_label, pre_train, 'Training', test_label, pre_test, 'Test')
saveas(gcf, "rho" + num2str(rho) + ".jpg")
 
%end


